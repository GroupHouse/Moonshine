    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		Calc.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				...
				//double TMP = Double.parseDouble(TempCorrection.getText().toString());
				//double correction;
				double Potential;
				double mashsize;
			    Spinner MeasureType = (Spinner) findViewById(R.id.spMeasure);
				...
				Potential = ((SA-EA)*1000 / 7.5);  //(((EA) * 220.5) + 1.6-2.5) * SA * 0.59;  //\\(SA - EA) / (7.75 - 3*(SA - 7) / 800);
				mashsize = MS * (Potential/90);
				
				PotAlc.setText(String.valueOf("Mash is " + fmt.format(Potential) + "% Alcohol") + String.valueOf("\n90% ABV = " + fmt.format(mashsize) + " " + MeasureType.getSelectedItem().toString() + "'s of Alcohol\n\n\n"));
